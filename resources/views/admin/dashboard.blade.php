...
<div class="min-h-screen bg-[#0b0b0c] text-gray-200 p-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-8 gap-4">
        <div class="flex items-center gap-4">
            <img src="{{ asset('javanese_wedding_couple.png') }}" class="w-16 h-16 object-contain" alt="Javanese Wedding Couple">
            <div>
        ...
        <div class="flex gap-3">
            <a href="{{ url('/admin/chat') }}" class="inline-flex items-center justify-center px-4 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold text-sm rounded-xl transition shadow-lg shadow-indigo-500/10">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
        <div class="bg-[#121214] border border-gray-800/70 p-6 rounded-2xl flex items-center justify-between">
            <div>
                ...
                <h3 class="text-2xl font-bold text-white mt-1">{{ $bookings->count() }}</h3>
            </div>
        ...
        <div class="bg-[#121214] border border-gray-800/70 p-6 rounded-2xl flex items-center justify-between">
            <div>
                ...
                <h3 class="text-2xl font-bold text-green-400 mt-1">
                    {{ $bookings->filter(fn($b) => strtolower($b->status ?? '') === 'confirmed')->count() }}
                </h3>
            </div>
        ...
        <div class="bg-[#121214] border border-gray-800/70 p-6 rounded-2xl flex items-center justify-between">
            <div>
                ...
                <h3 class="text-2xl font-bold text-blue-400 mt-1">
                    {{ $bookings->filter(fn($b) => in_array(strtolower($b->status ?? 'pending'), ['pending', '']))->count() }}
                </h3>
            </div>
        ...
        <div class="bg-[#121214] border border-gray-800/70 p-6 rounded-2xl flex items-center justify-between">
            <div>
                ...
                <h3 class="text-2xl font-bold text-red-400 mt-1">
                    {{ $bookings->filter(fn($b) => strtolower($b->status ?? '') === 'cancelled')->count() }}
                </h3>
            </div>
                    ...
                    @forelse($bookings as $booking)
                        <tr class="hover:bg-white/[0.01] transition duration-150">
                            ...
                            <td class="px-6 py-4 font-semibold text-white">
                                <i class="fa-solid fa-hotel text-gray-500 me-1"></i> {{ $booking->nama_ruangan }}
                            </td>
                            <td class="px-6 py-4 text-gray-300">
                                <i class="fa-solid fa-calendar text-gray-500 me-1"></i> {{ date('d M Y', strtotime($booking->tanggal_acara ?? $booking->created_at)) }}
                            </td>
                            <td class="px-6 py-4 text-xs text-gray-500">
                                <i class="fa-solid fa-clock text-gray-500 me-1"></i> {{ date('d/m/Y H:i', strtotime($booking->created_at)) }}
                            </td>
