...
<body class="bg-[#0b0b0b] text-white antialiased">
    ...
    <nav class="max-w-7xl mx-auto px-6 py-5 flex items-center justify-between">
        <div class="flex items-center gap-12">
            <a href="{{ url('/home') }}" class="text-2xl font-bold tracking-tight text-white">
            ...
            @if(auth()->check() && auth()->user()->role === 'admin')
                <div class="hidden md:flex items-center gap-8 text-gray-400 text-sm font-medium">
                    <a href="{{ url('/admin/dashboard') }}" class="{{ request()->is('admin/dashboard') ? 'text-white' : 'hover:text-white transition' }}">Dashboard</a>
                    <a href="{{ url('/admin/bookings') }}" class="{{ request()->is('admin/bookings') ? 'text-white' : 'hover:text-white transition' }}">Data Booking</a>
                    <a href="{{ url('/jadwal') }}" class="{{ request()->is('jadwal') ? 'text-white' : 'hover:text-white transition' }}">Jadwal</a>
                </div>
            @else
                <div class="hidden md:flex items-center gap-8 text-gray-400 text-sm font-medium">
                    <a href="{{ url('/home') }}" class="{{ request()->is('home') ? 'text-white' : 'hover:text-white transition' }}">Home</a>
                    <a href="{{ url('/booking') }}" class="{{ request()->is('booking') ? 'text-white' : 'hover:text-white transition' }}">Booking</a>
                    <a href="{{ url('/jadwal') }}" class="{{ request()->is('jadwal') ? 'text-white' : 'hover:text-white transition' }}">Jadwal</a>
                </div>
            @endif
        </div>
        ...
        <div class="flex items-center gap-5">
            <a href="{{ url('/jadwal') }}" class="text-sm font-medium hover:text-gray-300 transition">Cek Tanggal</a>
            
            @guest
                <div class="flex items-center gap-4 border-l border-gray-800 pl-4">
                    <a href="{{ url('/login') }}" class="text-sm font-medium text-gray-300 hover:text-white transition">Login</a>
                    <a href="{{ url('/register') }}" class="text-sm font-medium border border-gray-700 hover:border-gray-500 px-4 py-2 rounded-full transition">Sign Up</a>
                </div>
            @else
                <div class="flex items-center gap-4 border-l border-gray-800 pl-4">
                    <!-- Notification Bell -->
                                ...
                                @forelse($userNotifications as $notif)
                                    ...
                                    <div class="px-4 py-3 hover:bg-white/[0.02] transition">
                                        <div class="flex items-center justify-between mb-1">
                                            <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-semibold {{ $isConfirmed ? 'bg-green-500/10 text-green-400 border border-green-500/20' : 'bg-red-500/10 text-red-400 border border-red-500/20' }}">
                                                {{ $isConfirmed ? 'Dikonfirmasi' : 'Dibatalkan' }}
                                            </span>
                                            <span class="text-xs text-gray-500">{{ date('d M Y', strtotime($notif->updated_at)) }}</span>
                                        </div>
                                        <div class="font-semibold text-white mt-1">#BK-{{ str_pad($notif->id, 4, '0', STR_PAD_LEFT) }}</div>
                                        <div class="text-xs text-gray-400 mt-1 font-medium">{{ $notif->nama_ruangan }}</div>
                    ...
                    <span class="text-sm text-gray-300 font-medium">
                        <i class="fa-solid fa-user mr-1 text-[#f0b852]"></i> {{ auth()->user()->name }}
                    </span>
                    <form action="{{ route('logout') }}" method="POST" id="logout-form-app" class="hidden">
                        @csrf
                    </form>
            ...
            <a href="{{ url('/booking') }}" class="bg-[#f0b852] text-black text-sm font-semibold px-5 py-2.5 rounded-full hover:bg-[#dba33f] transition">Book Sekarang</a>
        </div>
    </nav>
